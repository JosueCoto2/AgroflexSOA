...
CREATE TABLE `cosechas_lote` (
  ...
  `variedad` varchar(100) DEFAULT NULL COMMENT 'Ej: Tomate Saladette, Saladet...',
  `cantidad_total` decimal(12,3) DEFAULT NULL,
  `cantidad_disponible` decimal(12,3) DEFAULT NULL,
  `unidad_venta` varchar(30) DEFAULT NULL,
  `precio_unitario` decimal(10,2) NOT NULL COMMENT 'Precio por unidad_venta',
  `precio_minimo_negoc` decimal(10,2) DEFAULT NULL COMMENT 'Precio mínimo negociable (confidencial)',
  `moneda` varchar(10) DEFAULT NULL,
  `grado_calidad` varchar(20) DEFAULT NULL,
  `es_organico` tinyint(1) NOT NULL DEFAULT 0,
  `certificaciones` varchar(255) DEFAULT NULL COMMENT 'JSON array de certificaciones',
  `fecha_cosecha` date DEFAULT NULL,
  `fecha_disponible` date NOT NULL,
  `fecha_vencimiento` date DEFAULT NULL COMMENT 'Fecha límite de disponibilidad',
  ...
  `direccion_parcela` varchar(255) DEFAULT NULL,
  `estado_lote` varchar(20) DEFAULT NULL,
  `vector_embedding` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'Embedding de OpenAI para búsqueda semántica' CHECK (json_valid(`vector_embedding`)),
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `deleted_at` datetime DEFAULT NULL,
  `nombre_productor` varchar(200) DEFAULT NULL,
  `reputacion_productor` decimal(3,2) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Lotes de cosecha publicados por productores e invernaderos';
...
ALTER TABLE `cosechas_lote`
  ...
  ADD KEY `idx_cl_estado` (`estado_lote`),
  ADD KEY `idx_cl_productor` (`id_productor`),
  ADD KEY `idx_cl_cultivo` (`id_cultivo`),
  ADD KEY `idx_cl_ubicacion` (`estado_republica`,`municipio`),
  ADD KEY `idx_cl_disponible` (`fecha_disponible`),
  ADD KEY `idx_cl_precio` (`precio_unitario`),
  ADD KEY `idx_cl_geo` (`latitud`,`longitud`),
  ADD KEY `idx_cl_filtros` (`id_cultivo`,`estado_lote`,`precio_unitario`);
